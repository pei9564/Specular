Feature: 刪除 LLM 供應商配置
  ...
  Rule: 管理員可以刪除未被 Agent 使用的模型配置
    ...
    Example: 成功 - 刪除未使用的配置
      ...
      When 使用者發送 DELETE 請求至 "/api/v1/llm-models/model-004"
    ...
    Example: 失敗 - 模型不存在
      When 使用者發送 DELETE 請求至 "/api/v1/llm-models/non-existent"
  ...
  Rule: 被 Agent 使用的模型需特別處理才能刪除
    ...
    Example: 失敗 - 無法直接刪除被使用的模型
      ...
      When 使用者發送 DELETE 請求至 "/api/v1/llm-models/model-002"
  ...
  Rule: 預設模型需先取消預設才能刪除
    ...
    Example: 失敗 - 無法刪除預設模型
      ...
      When 使用者發送 DELETE 請求至 "/api/v1/llm-models/model-001"
      Then 請求應失敗，回傳狀態碼 400
  ...
  Rule: 支援軟刪除以便日後恢復
    ...
    Example: 成功 - 恢復軟刪除的模型
      ...
      When 使用者發送 POST 請求至 "/api/v1/llm-models/model-004/restore"
      Then 請求應成功
      And model_providers 表中 model-004 應更新:
        | field      | value |
        | is_deleted | false |
