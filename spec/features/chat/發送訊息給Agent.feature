Feature: 發送訊息給 Agent
  ...
  Rule: 用戶可以發送訊息並接收 Agent 回應
    ...
    Example: 成功 - 發送訊息並接收回應
      ...
      When 使用者發送 POST 請求至 "/api/conversations/conv-001/messages":
        | content | 你好，請問你是誰？ |
  ...
  Rule: 支援 AGUI 標準 Server-Sent Events 串流回應
    ...
    Example: 成功 - 串流模式發送文字訊息 (AGUI Standard)
      When 使用者發送 POST 請求至 "/api/conversations/conv-001/messages/stream":
        | content | 你好 |
      Then 系統應建立 SSE 連線
      And 回傳標頭應包含:
        | header        | value             |
  ...
  Rule: 系統應驗證訊息內容
    ...
    Example: 失敗 - 訊息過長
      When 使用者發送訊息，長度超過 32000 字元
      Then 請求應失敗，回傳狀態碼 400
      And 錯誤訊息應為 "Message content exceeds maximum length (32000 characters)"

    Example: 失敗 - 會話不存在
      When 使用者發送 POST 請求至 "/api/conversations/non-existent/messages":
        | content | Hello |
